#!/usr/bin/env node
// Copy the repo's datasets into the two package trees and stamp one CalVer
...
const TARGETS = [path.join(HERE, 'python/src/marbella_prices/data'), path.join(HERE, 'npm/data')];
...
const VERSION_FILES = [
  [path.join(HERE, 'python/pyproject.toml'), /^version = "[^"]+"/m, (v) => `version = "${v}"`],
  [path.join(HERE, 'python/src/marbella_prices/__init__.py'), /^__version__ = "[^"]+"/m, (v) => `__version__ = "${v}"`],
  [path.join(HERE, 'npm/package.json'), /"version": "[^"]+"/, (v) => `"version": "${v}"`],
];
const versions = new Set();
for (const [file, re, render] of VERSION_FILES) {
  const text = readFileSync(file, 'utf8');
  const m = text.match(re);
  if (!m) { console.error(`no version field in ${path.relative(ROOT, file)}`); process.exit(2); }
  if (versionArg && !check) {
    writeFileSync(file, text.replace(re, render(versionArg)));
    console.log(`stamped ${versionArg} into ${path.relative(ROOT, file)}`);
  } else {
    versions.add(m[0].replace(/.*"([^"]+)".*/, '$1'));
  }
}
